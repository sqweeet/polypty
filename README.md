...
| Вид | Состояние |
|---|---|
| `codex` + мягкий glint обеих строк | агент выполняет задачу |
| `codex · blocked` | агент ждёт подтверждения или ответа |
| `codex` + зелёный бейдж `READY` справа | агент готов к следующей команде |

Во время работы один широкий приглушённый блик плавно проходит через фон обеих
строк как через цельную карточку; текст, путь и геометрия не анимируются. В
состоянии готовности справа появляется зелёная табличка `READY` с тёмным
текстом. `blocked` остаётся красным и явным. Если во вкладке несколько split
panes, наверх поднимается самый важный статус: `blocked` → `working` → `ready`,
включая неактивные pane.
