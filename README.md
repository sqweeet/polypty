...
| Вид | Состояние |
|---|---|
| `codex` + мягкий glint фона | агент выполняет задачу |
| `codex · blocked` | агент ждёт подтверждения или ответа |
| `codex · ready` | агент готов к следующей команде |

Во время работы по фону строки имени проходит одна приглушённая тёплая полоса;
текст, путь и геометрия карточки не анимируются. `blocked` подсвечивается
красным, `ready` — зелёным. Если во вкладке несколько split panes, наверх
поднимается самый важный статус: `blocked` → `working` → `ready`, включая
неактивные pane.
