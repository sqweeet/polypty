...
В состоянии готовности справа появляется компактный зелёный `✓`. Если во
вкладке несколько split panes с одинаковым агентом, карточка объединяет их как
`codex ×2`; смешанный набор показывается через главный агент и число остальных,
например `claude+1`. Наверх поднимается самый важный статус: `blocked` →
`working` → `ready`, включая неактивные pane. `blocked` остаётся красным и
явным.
