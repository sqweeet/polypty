# Конфигурация polypty

`polypty` читает TOML-конфиг при запуске. Основной путь:

```text
$XDG_CONFIG_HOME/polypty/config.toml
```

Если `XDG_CONFIG_HOME` не задан, используется
`~/.config/polypty/config.toml`. Для разового запуска другой файл можно указать
через `POLYPTY_CONFIG=/path/to/config.toml`. Отсутствующий основной конфиг не
является ошибкой: сохраняются встроенные настройки.
Изменения применяются при следующем запуске `polypty`; live reload пока нет.
...
Одинаковый пользовательский chord нельзя назначить двум действиям: `polypty`
